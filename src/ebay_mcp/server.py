...
def build_server(config: Config | None = None, client: EbayClient | None = None) -> FastMCP:
    ...
    mcp = FastMCP(
        ...
        instructions=(
            ...
            "unless noted, and are isolated to a single currency. A listing whose "
            "shipping cost eBay did not report has `total_cost: null` and "
            "`shipping_known: false`; set EBAY_DELIVERY_POSTAL_CODE so eBay "
            "calculates shipping. Search tools "
